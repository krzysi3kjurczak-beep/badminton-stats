...
const PLAN_VENUES = [
  {
    id: 'csr-gdansk',
    name: 'Centrum Sportów Rakietowych Gdańsk',
    mapsQuery: 'Centrum Sportów Rakietowych Gdańsk',
  },
  {
    id: 'kbwg',
    name: 'Klub Badmintona w Garnizonie',
    mapsQuery: 'Klub Badmintona w Garnizonie',
  },
  {
    id: 'sport-park-przymorze',
    name: 'Sport Park Przymorze',
    mapsQuery: 'Sport Park Przymorze',
  },
];
const MAX_PLAN_COURTS = 6;
const MAX_PLAN_COURT_LABEL_LEN = 32;
...
function canCreateMatch() {
  if (isSpectatorMode() || isMatchSpectatorMode() || isMatchRefereeMode()) return false;
...
function isAssignedReferee(m) {
  return !!(m && userSession.playerId != null && m.refereePlayerId === userSession.playerId);
}
...
function isRefereeLinkSessionForMatch(matchId) {
  return getRefereeSession()?.matchId === matchId;
}

function getAssignedRefereeMatchId() {
  const m = matches.find(x => isAssignedReferee(x) && isMatchActive(x) && !isMatchEditMode(x));
  return m?.id ?? null;
}

function hasRefereeLinkForMatch(match) {
  if (!match || !isRefereeLinkSessionForMatch(match.id)) return false;
...
function getRefereeSession() {
  try {
    const raw = sessionStorage.getItem(REFEREE_SESSION_KEY);
...
function setRefereeSession(data) {
  try {
    sessionStorage.setItem(REFEREE_SESSION_KEY, JSON.stringify(data));
  } catch (_) {}
}

function clearRefereeSession() {
  try {
    sessionStorage.removeItem(REFEREE_SESSION_KEY);
  } catch (_) {}
}

function isRefereeLinkSession() {
...
function getRefereeTargetMatchId() {
  ...
  return getAssignedRefereeMatchId();
}
...
function isRefereeFlowActive() {
  ...
  if (session?.matchId === id) {
    const m = matches.find(x => x.id === id);
    if (!m) return true;
    if (!isMatchActive(m) || isMatchEditMode(m)) return false;
    return hasRefereeLinkForMatch(m);
  }
  const m = matches.find(x => x.id === id);
  if (!m) return false;
  return isMatchActive(m) && !isMatchEditMode(m) && isAssignedReferee(m);
}
...
function isMatchRefereeMode(m) {
  const id = m?.id ?? openMatchId ?? getRefereeTargetMatchId();
  if (!id) return false;
  const match = m ?? matches.find(x => x.id === id);
  if (!match) return false;
  if (hasRefereeLinkForMatch(match)) return true;
...
function requireMatchScore(m) {
  if (!canScoreMatch(m)) {
    showToast('Brak uprawnień do wpisywania punktów', 'warn');
    return false;
  }
  return true;
}
...
function requireLiveMatchControl(m) {
  if (!canControlLiveMatch(m)) {
    showToast('Brak uprawnień do tej akcji', 'warn');
    return false;
  }
  return true;
}

function isRefereeUiLocked(m) {
  const matchId = m?.id ?? openMatchId ?? getRefereeTargetMatchId();
  const match = m ?? matches.find(x => x.id === matchId);
  if (!match || !isMatchRefereeMode(match)) return false;
...
let refereeNamePromptOpen = false;
let pendingRefereeMatchId = null;
...
function mergeMatchByUpdatedAt(local, remote) {
  ...
  if (local.status === 'finished' && remote.status === 'active') {
    return finalizeMergedMatch(local, remote, { ...local, updatedAt: Math.max(lT, rT) });
  }
  if (remote.status === 'finished' && local.status === 'active') {
...
function applyLeagueState(data, opts = {}) {
  ...
  if (ver < 17) {
    matches = matches.map(m => ({
      ...
      refereeRequestPlayerId: m.refereeRequestPlayerId ?? null,
    }));
  }
  ...
  if (ver < 18) {
    matches = matches.map(m => ({
      ...m,
      refereeGuest: m.refereeGuest ?? false,
    }));
  }
  ...
  if (ver < 19) {
    matches = matches.map(m => ({
      ...m,
      refereeGuestName: m.refereeGuestName ?? null,
    }));
  }
...
function loadState() {
  try {
    ...
    if (raw) {
      ...
      let needsSave = !data.stateVersion || data.stateVersion < STATE_VERSION;
      ...
      if (autoArchivePastPlannedSessions()) needsSave = true;
...
function saveUiState() {
  try {
    sessionStorage.setItem(UI_STATE_KEY, JSON.stringify({
      ...
      planningArchivedOpen: currentTab === 'matches' && matchesRosterTab === 'planning' ? planningArchivedOpen : false,
      profileOpen,
...
function restoreUiState() {
  try {
    ...
    if (typeof data.planningArchivedOpen === 'boolean' && currentTab === 'matches' && matchesRosterTab === 'planning') {
      planningArchivedOpen = data.planningArchivedOpen;
    }
...
function syncBottomNav() {
  ...
  document.getElementById('app')?.classList.toggle('app--referee', refereeLocked);
}
...
async function ensureRefereeLeagueSync() {
  ...
  try {
    ...
    console.warn('referee league sync failed', err);
    return false;
  }
}
...
function getRefereeMatchUrl(matchId) {
  return `${getAppShareUrl()}?referee=${matchId}`;
}

function buildRefereeInvitePayload(m) {
  ...
  return {
    kind: 'referee',
    matchId: m.id,
    title: `Sędziuj: ${nameA} vs ${nameB}`,
    text: `Sędziuj mecz badmintona na żywo: ${nameA} vs ${nameB}.`,
    url: getRefereeMatchUrl(m.id),
    bannerTag: 'Sędziowanie na żywo',
    bannerHeadline: `${nameA} vs ${nameB}`,
    bannerSub: 'Kliknij link i sędziuj mecz',
  };
}

function parseRefereeFromUrl() {
  const params = new URLSearchParams(window.location.search);
  const matchId = parseInt(params.get('referee') || '', 10);
...
function primeRefereeEntry(matchId, extras = {}) {
  const prev = getRefereeSession();
  setRefereeSession({
    matchId,
    joinedAt: Date.now(),
    claimedGuest: prev?.matchId === matchId ? prev.claimedGuest : undefined,
    ...extras,
  });
  currentTab = 'matches';
  openMatchId = matchId;
...
function tryClaimRefereeRole(m) {
  if (!m || !isRefereeLinkSessionForMatch(m.id)) return { ok: false, reason: 'nosession' };
  if (m.status === 'finished' || !isMatchActive(m) || isMatchEditMode(m)) {
  ...
  if (userSession.loggedIn && userSession.playerId) {
    ...
    m.refereePlayerId = userSession.playerId;
    ...
    return { ok: true, upgraded: changed };
  }
  ...
  m.refereeRequestPlayerId = null;
  const session = getRefereeSession();
  const guestName = session?.displayName?.trim();
...
function upgradeRefereeAccountOnLogin() {
  const session = getRefereeSession();
  if (!session?.matchId || !userSession.playerId) return;
  const m = matches.find(x => x.id === session.matchId);
  if (!m || !isMatchActive(m) || isMatchEditMode(m)) return;
  if (m.refereePlayerId && m.refereePlayerId !== userSession.playerId) return;
  const changed = m.refereeGuest || !m.refereePlayerId;
  if (!changed) return;
  m.refereePlayerId = userSession.playerId;
  m.refereeGuest = false;
  ...
  setRefereeSession({
    ...
    claimedGuest: session.claimedGuest,
  });
  const name = getPlayer(userSession.playerId)?.displayName || 'Ty';
  showToast(`Zalogowano — sędziujesz jako ${name}`, 'ok');
}

function dismissInvalidRefereeEntry(reason, matchId) {
  if (reason === 'finished') {
    showToast('Ten mecz jest już zakończony — link sędziowski wygasł', 'warn');
  } else if (reason === 'taken') {
    showToast('Ten mecz ma już innego sędziego', 'warn');
  }
...
function ensureRefereeLiveUi(m) {
  if (!isMatchRefereeMode(m) || openMatchId !== m.id) return false;
  let changed = false;
  if (isServeDuelActive(m)) {
    if (servePickerMatchId !== m.id) {
  ...
  if (m.liveSet && isMatchActive(m) && !reopenMatchEdit && !isMatchEditMode(m)) {
    if (!setPlayOpen) {
      setPlayOpen = true;
      changed = true;
    }
...
function handleRefereeLeagueSync() {
  ...
  if (isRefereeLinkSessionForMatch(targetId)) {
    const claim = tryClaimRefereeRole(m);
    if (!claim.ok) {
  ...
  currentTab = 'matches';
  const wasOnTarget = openMatchId === targetId && document.querySelector('.match-page');
...
function requestRefereeRole(m) {
  if (!m || !userSession.loggedIn || !hasAuthAccount()) return;
  if (isMatchParticipant(m) || isMatchRefereeMode(m)) return;
  if (!isMatchLiveActive(m) || isMatchEditMode(m)) return;
  if (hasActiveReferee(m)) {
    showToast('Ten mecz ma już sędziego', 'warn');
    return;
  }
  if (m.refereeRequestPlayerId && m.refereeRequestPlayerId !== userSession.playerId) {
    showToast('Ktoś inny już prosi o sędziowanie tego meczu', 'warn');
    return;
  }
...
function renderRefereeRequestBanner(m) {
  if (hasActiveReferee(m) || !m?.refereeRequestPlayerId) return '';
  const requester = getPlayer(m.refereeRequestPlayerId);
  const name = requester?.displayName || 'Zawodnik';
  if (canEditMatch(m)) {
    return `
      <div class="referee-request-banner">
        <p class="referee-request-banner__text"><strong>${escAttr(name)}</strong> chce sędziować ten mecz.</p>
        <div class="referee-request-banner__actions">
          <button class="btn btn--primary btn--sm" data-action="approve-referee" type="button">Zgadzam się</button>
          <button class="btn btn--outline btn--sm" data-action="reject-referee" type="button">Odrzuć</button>
        </div>
      </div>`;
  }
  if (m.refereeRequestPlayerId === userSession.playerId) {
    return `<p class="referee-request-banner referee-request-banner--pending">Oczekujesz na zgodę uczestników meczu…</p>`;
  }
  return '';
}
...
async function confirmRefereeEntry(name) {
  const matchId = pendingRefereeMatchId || getRefereeTargetMatchId();
  if (!matchId) return;
  const trimmed = String(name || '').trim().slice(0, 40);
  if (!trimmed) {
    showToast('Podaj imię lub nick sędziego', 'warn');
    return;
  }
  let m = matches.find(x => x.id === matchId);
  if (!m) {
    await ensureRefereeLeagueSync();
  ...
  const claim = tryClaimRefereeRole(m);
  if (!claim.ok) {
    dismissInvalidRefereeEntry(claim.reason, matchId);
    return;
  }
...
function getPlannedSession(id) {
  const nid = Number(id);
  const session = plannedSessions.find(s => Number(s.id) === nid) || null;
  if (!session || session.status === 'deleted' || session.status === 'cancelled') return null;
  return session;
}
...
function getPlanVenue(placeId) {
  return PLAN_VENUES.find(v => v.id === placeId) || null;
}

function getPlanVenueName(placeId) {
  return getPlanVenue(placeId)?.name || 'Miejsce';
}

function getPlanVenueMapsUrl(placeId) {
  const venue = getPlanVenue(placeId);
  if (!venue) return null;
  const query = encodeURIComponent(venue.mapsQuery || venue.name);
  return `https://www.google.com/maps/search/?api=1&query=${query}`;
}

async function openPlanVenueMaps(placeId) {
  const url = getPlanVenueMapsUrl(placeId);
  if (!url) return;
  window.open(url, '_blank', 'noopener,noreferrer');
}

function planPlayerIdEq(a, b) {
  if (a == null || b == null) return false;
  return Number(a) === Number(b);
}

function planPlayerIdInList(list, playerId) {
  return (list || []).some(id => planPlayerIdEq(id, playerId));
}

function normalizePlannedSession(session) {
  if (!session) return session;
  const format = session.defaultFormat === 'doubles' ? 'doubles' : 'singles';
  const size = format === 'doubles' ? 2 : 1;
  session.pool = Array.isArray(session.pool) ? session.pool.map(id => Number(id)).filter(id => !Number.isNaN(id)) : [];
  session.slots = (session.slots || []).map(slot => {
    const next = normalizePlanSlot(slot, slot.format === 'doubles' ? 'doubles' : format);
    next.teamA = (next.teamA || []).map(id => (id == null ? null : Number(id)));
    next.teamB = (next.teamB || []).map(id => (id == null ? null : Number(id)));
    while (next.teamA.length < size) next.teamA.push(null);
    while (next.teamB.length < size) next.teamB.push(null);
    next.teamA = next.teamA.slice(0, size);
    next.teamB = next.teamB.slice(0, size);
    return next;
  });
  return session;
}

function autoArchivePastPlannedSessions() {
  const now = Date.now();
  let changed = false;
  plannedSessions.forEach(s => {
    normalizePlannedSession(s);
    if (s.status !== 'open' && s.status !== 'started') return;
    const t = Date.parse(s.scheduledAt);
    if (t && t < now) {
      s.status = 'archived';
      touchPlannedSessionUpdated(s);
      changed = true;
    }
  });
  return changed;
...
function findPlanSlot(session, slotId) {
  const id = Number(slotId);
  return (session?.slots || []).find(s => Number(s.id) === id) || null;
}
...
function normalizePlanSlot(slot, format) {
  const size = format === 'doubles' ? 2 : 1;
  const padTeam = arr => {
    const next = [...(arr || [])];
  ...
  return {
    ...slot,
    format,
    teamA: padTeam(slot.teamA),
    teamB: padTeam(slot.teamB),
  };
}

function resizePlannedSessionSlots(session, newCourtCount, format) {
  const existing = session.slots || [];
  const newSlots = [];
  for (let i = 0; i < newCourtCount; i++) {
    const id = i + 1;
    const prev = existing.find(s => Number(s.id) === id);
    if (prev?.startedMatchId) {
      newSlots.push(prev);
      continue;
    }
    if (prev) {
      newSlots.push(normalizePlanSlot(prev, format));
      continue;
    }
    newSlots.push({
      id,
      label: newCourtCount > 1 ? `Kort ${id}` : 'Kort',
      format,
      teamA: Array(format === 'doubles' ? 2 : 1).fill(null),
      teamB: Array(format === 'doubles' ? 2 : 1).fill(null),
    });
  }
  existing.forEach(slot => {
    if (Number(slot.id) > newCourtCount && !slot.startedMatchId) {
      session.pool = session.pool || [];
      getPlanSlotPlayerIds(slot).forEach(pid => {
        if (!session.pool.includes(pid)) session.pool.push(pid);
      });
    }
  });
  session.slots = newSlots;
  session.courtCount = newCourtCount;
  session.defaultFormat = format;
}
...
function isPlannedSessionOrganizer(session) {
  return !!(session && userSession.playerId != null && Number(session.createdByPlayerId) === Number(userSession.playerId));
}

function canManagePlannedSession(session) {
  return isPlannedSessionOrganizer(session) || isAppAdmin();
}
...
function isPlayerInPlannedSession(session, playerId) {
  ...
  if (planPlayerIdInList(session.pool, playerId)) return true;
  return (session.slots || []).some(slot => getPlanSlotPlayerIds(slot).some(id => planPlayerIdEq(id, playerId)));
}

function removePlayerFromPlanSlots(session, playerId) {
  (session.slots || []).forEach(slot => {
    slot.teamA = slot.teamA.map(id => (planPlayerIdEq(id, playerId) ? null : id));
    slot.teamB = slot.teamB.map(id => (planPlayerIdEq(id, playerId) ? null : id));
  });
}
...
function assignPlanPlayer(session, playerId, slotId, side, index, { fromPool = false } = {}) {
  if (!canManagePlannedSession(session)) return false;
  normalizePlannedSession(session);
  playerId = Number(playerId);
  index = Number(index);
  slotId = Number(slotId);
  if (!getPlayer(playerId) || Number.isNaN(playerId) || Number.isNaN(index)) return false;
  ...
  if (fromPool) {
    const poolIdx = (session.pool || []).findIndex(id => planPlayerIdEq(id, playerId));
    if (poolIdx === -1) return false;
    session.pool.splice(poolIdx, 1);
  } else {
    removePlayerFromPlanSlots(session, playerId);
    const poolIdx = (session.pool || []).findIndex(id => planPlayerIdEq(id, playerId));
    if (poolIdx !== -1) session.pool.splice(poolIdx, 1);
  }
  team[index] = playerId;
  touchPlannedSessionUpdated(session);
...
function assignPlanPoolPlayer(session, playerId, slotId, side, index) {
  return assignPlanPlayer(session, playerId, slotId, side, index, { fromPool: true });
}

function assignPlanRosterPlayer(session, playerId, slotId, side, index) {
  return assignPlanPlayer(session, playerId, slotId, side, index, { fromPool: false });
}

function removePlayerFromPlannedSession(session, playerId, { returnToPool = false } = {}) {
  if (!session || playerId == null) return false;
  normalizePlannedSession(session);
  const wasOnSlot = (session.slots || []).some(slot => getPlanSlotPlayerIds(slot).some(id => planPlayerIdEq(id, playerId)));
  removePlayerFromPlanSlots(session, playerId);
  session.pool = (session.pool || []).filter(id => !planPlayerIdEq(id, playerId));
  if (returnToPool && wasOnSlot && session.courtCount > 1) {
    if (!planPlayerIdInList(session.pool, playerId)) session.pool.push(Number(playerId));
  }
  touchPlannedSessionUpdated(session);
  return true;
}
...
function createPlannedSessionFromDraft() {
  ...
  draft.dateError = '';
  draft.timeError = '';
  const courtCount = Math.min(MAX_PLAN_COURTS, Math.max(1, draft.courtCount || 1));
...
function archivePlannedSession(session) {
  if (!canManagePlannedSession(session)) return false;
...
function getActivePlanningSessions() {
  autoArchivePastPlannedSessions();
  return plannedSessions
    .filter(s => s.status === 'open' || s.status === 'started')
    .sort((a, b) => (Date.parse(a.scheduledAt) || 0) - (Date.parse(b.scheduledAt) || 0));
}

function getArchivedPlanningSessions() {
  autoArchivePastPlannedSessions();
  return plannedSessions
...
function isPlanningSessionVisible(session) {
  return session && session.status !== 'deleted' && session.status !== 'cancelled';
}

function plannedSessionEditDefault(session) {
  const d = new Date(session.scheduledAt);
  const pad = n => String(n).padStart(2, '0');
  return {
    date: Number.isNaN(d.getTime()) ? todayIso() : d.toISOString().slice(0, 10),
    time: Number.isNaN(d.getTime()) ? '18:00' : `${pad(d.getHours())}:${pad(d.getMinutes())}`,
    placeId: session.placeId || PLAN_VENUES[0].id,
    courtCount: session.courtCount || 1,
    format: session.defaultFormat === 'doubles' ? 'doubles' : 'singles',
    dateError: '',
    timeError: '',
  };
}

function syncPlanEditDraftFromDom() {
  if (!planEditDraft) return;
  const dateEl = document.getElementById('plan-edit-date');
  const timeEl = document.getElementById('plan-edit-time');
  const placeEl = document.getElementById('plan-edit-place');
  const courtsEl = document.getElementById('plan-edit-courts');
  if (dateEl) planEditDraft.date = dateEl.value;
  if (timeEl) planEditDraft.time = timeEl.value;
  if (placeEl) planEditDraft.placeId = placeEl.value;
  if (courtsEl) planEditDraft.courtCount = parseInt(courtsEl.value, 10) || 1;
}
...
function savePlannedSessionEdit(sessionId) {
  ...
  const courtCount = Math.min(MAX_PLAN_COURTS, Math.max(1, draft.courtCount || 1));
  const format = draft.format === 'doubles' ? 'doubles' : 'singles';
  session.scheduledAt = scheduledAt.toISOString();
  session.placeId = draft.placeId || PLAN_VENUES[0].id;
  resizePlannedSessionSlots(session, courtCount, format);
  touchPlannedSessionUpdated(session);
  planEditOpen = false;
  planEditDraft = null;
...
function savePlanSlotLabel(sessionId, slotId, rawLabel) {
  const session = getPlannedSession(sessionId);
  const slot = findPlanSlot(session, slotId);
  if (!session || !slot || !canManagePlannedSession(session) || slot.startedMatchId) return false;
  const trimmed = String(rawLabel || '').trim();
  const label = trimmed ? clampPlayerOrTeamName(trimmed).slice(0, MAX_PLAN_COURT_LABEL_LEN) : (session.courtCount > 1 ? `Kort ${slot.id}` : 'Kort');
  if (label !== slot.label) {
    slot.label = label;
    touchPlannedSessionUpdated(session);
    saveState();
...
function updateNewPlannedFormatDOM() {
  const draft = newPlannedDraft;
  if (!draft) return;
  const isDoubles = draft.format === 'doubles';
  document.querySelectorAll('[data-action="set-plan-format"]').forEach(btn => {
    const active = btn.dataset.format === (isDoubles ? 'doubles' : 'singles');
    btn.classList.toggle('new-match__type-btn--active', active);
  });
}

function updatePlanEditFormatDOM() {
  const draft = planEditDraft;
  if (!draft) return;
  const isDoubles = draft.format === 'doubles';
  document.querySelectorAll('[data-action="set-plan-edit-format"]').forEach(btn => {
    const active = btn.dataset.format === (isDoubles ? 'doubles' : 'singles');
    btn.classList.toggle('new-match__type-btn--active', active);
  });
}
...
async function enableAppNotifications() {
  ...
  const sub = (window.APP_CONFIG?.vapidPublicKey)
    ? await BadmintonPush.subscribePush({ force: true })
    : null;
...
function getPlanningSessionsForDisplay() {
  return getActivePlanningSessions();
}

function softUpdatePlanningList() {
  const page = document.querySelector('.matches-page');
  if (!page || matchesRosterTab !== 'planning' || openPlannedSessionId) return false;
  const archived = getArchivedPlanningSessions();
  const active = getActivePlanningSessions();
  const archivedPanel = page.querySelector('.plan-archived-panel');
  const archivedList = page.querySelector('.plan-archived-list');
  const activeList = page.querySelector('.plan-list--active');
  ...
  if (archivedPanel) {
    ...
    if (archivedList) {
      archivedList.hidden = !planningArchivedOpen;
      archivedList.innerHTML = archived.length
        ? archived.map(renderPlannedSessionCard).join('')
        : '<p class="match-detail__empty">Brak zarchiwizowanych planów</p>';
    }
  }
  return true;
}
...
function softUpdatePlannedSessionDetail(sessionId) {
  ...
  el.outerHTML = renderPlannedSessionDetail(session);
  return true;
}
...
function renderPlanField({ id, label, type, value, icon, error = '' }) {
  return `
    <label class="profile-card__label" for="${id}">${label}</label>
    <div class="plan-field">
      <span class="plan-field__icon" aria-hidden="true">${icon}</span>
      <input class="plan-field__input" type="${type}" id="${id}" value="${escAttr(value)}">
    </div>
    ${error ? `<p class="new-match__error">${escAttr(error)}</p>` : ''}`;
}

function renderPlanSelectField({ id, label, value, optionsHtml, icon = null }) {
  const iconHtml = icon ? `<span class="plan-field__icon" aria-hidden="true">${icon}</span>` : '';
  return `
    <label class="profile-card__label" for="${id}">${label}</label>
    <div class="plan-field plan-field--select${icon ? '' : ' plan-field--select-plain'}">
      ${iconHtml}
      <select class="plan-field__input plan-field__input--select" id="${id}">${optionsHtml}</select>
    </div>`;
}

function renderPlanFormatToggle(draft, actionName) {
  const isDoubles = draft.format === 'doubles';
  return `
    <p class="profile-card__label">Format</p>
    <div class="new-match__type-toggle" id="${actionName}-format-toggle">
      <button class="new-match__type-btn${!isDoubles ? ' new-match__type-btn--active' : ''}" data-action="${actionName}" data-format="singles" type="button">Singiel</button>
      <button class="new-match__type-btn${isDoubles ? ' new-match__type-btn--active' : ''}" data-action="${actionName}" data-format="doubles" type="button">Debel</button>
    </div>`;
}

function renderPlanCourtOptions(selected) {
  return Array.from({ length: MAX_PLAN_COURTS }, (_, i) => i + 1).map(n =>
    `<option value="${n}"${selected === n ? ' selected' : ''}>${n}</option>`
  ).join('');
}

function renderPlanVenueOptions(selectedId) {
  return PLAN_VENUES.map(v =>
    `<option value="${escAttr(v.id)}"${selectedId === v.id ? ' selected' : ''}>${escAttr(v.name)}</option>`
  ).join('');
}
...
function renderPlanSlotSeat(session, slot, side, index) {
  ...
  if (playerId) {
    return `<div class="plan-slot__seat plan-slot__seat--filled">${renderPlanPlayerChip(playerId, { removable: canManage && !slot.startedMatchId, sessionId: session.id })}</div>`;
  }
...
function renderPlanSlotLabel(session, slot) {
  const canManage = canManagePlannedSession(session) && !slot.startedMatchId;
  if (!canManage) return `<strong class="plan-slot__label">${escAttr(slot.label)}</strong>`;
  return `
    <input
      class="plan-slot__label-input"
      type="text"
      value="${escAttr(slot.label)}"
      maxlength="${MAX_PLAN_COURT_LABEL_LEN}"
      data-action="plan-slot-label"
      data-session-id="${session.id}"
      data-slot-id="${slot.id}"
      aria-label="Nazwa kortu"
    >`;
}
...
function renderPlanManageActions(session) {
  if (!canManagePlannedSession(session)) return '';
  if (!['open', 'started', 'archived'].includes(session.status)) return '';
  const editBtn = session.status === 'open'
    ? `<button class="plan-manage-actions__btn" data-action="open-plan-edit" data-session-id="${session.id}" type="button" aria-label="Edytuj">${EDIT_ICON}<span>Edytuj</span></button>`
    : '';
  const archiveBtn = session.status !== 'archived'
    ? `<button class="plan-manage-actions__btn" data-action="plan-archive-session" data-session-id="${session.id}" type="button" aria-label="Archiwizuj">${ARCHIVE_ICON}<span>Archiwizuj</span></button>`
    : '';
  return `
    <div class="plan-manage-actions">
      ${editBtn}
      ${archiveBtn}
      <button class="plan-manage-actions__btn plan-manage-actions__btn--danger" data-action="plan-delete-session" data-session-id="${session.id}" type="button" aria-label="Usuń">${TRASH_ICON}<span>Usuń</span></button>
    </div>`;
}

function renderPlanCtxActions(sessionId) {
  const session = getPlannedSession(sessionId);
  if (!session || !canManagePlannedSession(session)) return '';
  const editBtn = session.status === 'open'
    ? `<button class="ctx-actions__btn" data-action="ctx-plan-edit" data-session-id="${sessionId}" type="button" aria-label="Edytuj">${EDIT_ICON}</button>`
    : '';
  const archiveBtn = session.status !== 'archived'
    ? `<button class="ctx-actions__btn" data-action="ctx-plan-archive" data-session-id="${sessionId}" type="button" aria-label="Archiwizuj">${ARCHIVE_ICON}</button>`
    : '';
  return `
    <div class="ctx-actions ctx-actions--plan">
      ${editBtn}
      ${archiveBtn}
      <button class="ctx-actions__btn ctx-actions__btn--danger" data-action="ctx-plan-delete" data-session-id="${sessionId}" type="button" aria-label="Usuń">${TRASH_ICON}</button>
    </div>`;
}

function findPlannedSessionRaw(id) {
  return plannedSessions.find(s => Number(s.id) === Number(id)) || null;
}

function promptDeletePlannedSession(session) {
  if (!session || !canManagePlannedSession(session)) return;
  pendingConfirm = {
    title: 'Usunąć planowanie?',
    message: 'Plan zniknie z listy. Rozpoczęte mecze pozostaną w historii.',
    confirmLabel: 'Usuń',
    danger: true,
    resolve: ok => {
      if (ok && deletePlannedSession(session)) {
        if (Number(openPlannedSessionId) === Number(session.id)) {
          openPlannedSessionId = null;
          planEditOpen = false;
          planEditDraft = null;
          planAssignPicker = null;
        ...
        ctxTarget = null;
        showToast('Usunięto planowanie', 'info');
      }
      render();
    },
  };
...
function renderPlanningList() {
  ...
  return `
    ${hint}
    ${archived.length ? `
      <div class="plan-archived-panel">
        <button type="button" class="plan-archived-panel__toggle" data-action="toggle-planning-archived" aria-expanded="${planningArchivedOpen ? 'true' : 'false'}">
          Zarchiwizowane (${archived.length})
        </button>
        <div class="plan-archived-list plan-list"${planningArchivedOpen ? '' : ' hidden'}>${archived.map(renderPlannedSessionCard).join('')}</div>
      </div>` : ''}
    ...
    <div class="new-match-layer">
      <div class="new-match-layer__scroll">
        ...
        <div class="new-match-glass" id="new-plan-glass">
          ...
          <h2 class="new-match__title">Zaplanuj grę</h2>

          ${renderPlanField({ id: 'new-plan-date', label: 'Data', type: 'date', value: draft.date, icon: CALENDAR_ICON, error: draft.dateError })}
          ${renderPlanField({ id: 'new-plan-time', label: 'Godzina', type: 'time', value: draft.time, icon: CLOCK_ICON, error: draft.timeError })}
          ${renderPlanSelectField({ id: 'new-plan-place', label: 'Miejsce', value: draft.placeId, optionsHtml: renderPlanVenueOptions(draft.placeId), icon: PIN_ICON })}
          ${renderPlanSelectField({ id: 'new-plan-courts', label: 'Liczba kortów', value: draft.courtCount, optionsHtml: renderPlanCourtOptions(draft.courtCount) })}
          ${renderPlanFormatToggle(draft, 'set-plan-format')}

          <button class="btn btn--primary btn--full new-match__submit" data-action="create-planned-session" type="button">Zaplanuj</button>
        </div>
...
function renderPlanEditForm(session) {
  const draft = planEditDraft || plannedSessionEditDefault(session);
  return `
    <div class="new-match-layer plan-edit-layer">
      <div class="new-match-layer__scroll">
        <div class="form-picker-scroll-pad form-picker-scroll-pad--top" aria-hidden="true"></div>
        <div class="new-match-glass" id="plan-edit-glass">
          <button class="match-info-glass__close" data-action="close-plan-edit" type="button" aria-label="Zamknij">${CLOSE_ICON}</button>
          <h2 class="new-match__title">Edytuj planowanie</h2>

          ${renderPlanField({ id: 'plan-edit-date', label: 'Data', type: 'date', value: draft.date, icon: CALENDAR_ICON, error: draft.dateError })}
          ${renderPlanField({ id: 'plan-edit-time', label: 'Godzina', type: 'time', value: draft.time, icon: CLOCK_ICON, error: draft.timeError })}
          ${renderPlanSelectField({ id: 'plan-edit-place', label: 'Miejsce', value: draft.placeId, optionsHtml: renderPlanVenueOptions(draft.placeId), icon: PIN_ICON })}
          ${renderPlanSelectField({ id: 'plan-edit-courts', label: 'Liczba kortów', value: draft.courtCount, optionsHtml: renderPlanCourtOptions(draft.courtCount) })}
          ${renderPlanFormatToggle(draft, 'set-plan-edit-format')}

          <button class="btn btn--primary btn--full new-match__submit" data-action="save-plan-edit" data-session-id="${session.id}" type="button">Zapisz</button>
        </div>
        <div class="form-picker-scroll-pad form-picker-scroll-pad--bottom" aria-hidden="true"></div>
      </div>
    </div>`;
...
function getPlayer(id) {
  ...
  const nid = Number(id);
  return players.find(p => Number(p.id) === nid) || null;
}
...
function isServeDuelStarter(m) {
  ...
  if (canRefereeScore(m)) return true;
...
function emptyParticipantStats() {
  return finalizeParticipantStats(createParticipantStatsShell());
}

function pickParticipantStatsBucket(fullStats, formatView = 'combined') {
  if (!fullStats) return emptyParticipantStats();
  if (fullStats.matchesPlayed != null) return fullStats;
  if (formatView === 'singles') return fullStats.singles || emptyParticipantStats();
  if (formatView === 'doubles') return fullStats.doubles || emptyParticipantStats();
  return fullStats.combined || emptyParticipantStats();
}

function accumulateParticipantStatsForPlayer(rawBuckets, m, side, playerId) {
  const isD = isDoublesMatch(m);
  accumulateMatchForSide(rawBuckets.combined, m, side, { participantId: playerId });
  accumulateMatchForSide(isD ? rawBuckets.doubles : rawBuckets.singles, m, side, { participantId: playerId });
}

function finalizeParticipantStatsBuckets(rawBuckets) {
  return {
    singles: finalizeParticipantStats(rawBuckets.singles),
    doubles: finalizeParticipantStats(rawBuckets.doubles),
    combined: finalizeParticipantStats(rawBuckets.combined),
  };
}

function createParticipantStatsBuckets() {
  return {
    singles: createParticipantStatsShell(),
    doubles: createParticipantStatsShell(),
    combined: createParticipantStatsShell(),
  };
}
...
function updateHeaderAvatar() {
  ...
  if (profileBtn) {
    ...
    profileBtn.classList.toggle('top-bar__avatar-btn--disabled', isRefereeUiLocked());
    profileBtn.disabled = isRefereeUiLocked();
    if (!userSession.loggedIn) {
...
function computePlayerStats(playerId) {
  const raw = createParticipantStatsBuckets();
  matches.forEach(m => {
    const side = playerSideInMatch(playerId, m);
    if (!side) return;
    accumulateParticipantStatsForPlayer(raw, m, side, playerId);
  });
  return finalizeParticipantStatsBuckets(raw);
}

function createGlobalStatsBucket() {
  return {
    matches: 0,
    finishedMatches: 0,
    activeMatches: 0,
...
function finalizeGlobalStatsBucket(b) {
  const avgPointsPerSet = b.totalSets > 0 ? (b.totalPoints / b.totalSets).toFixed(1) : '0.0';
  const avgSetDurationSec = b.totalSets > 0 ? Math.round(b.totalPlaySec / b.totalSets) : 0;
  const playMin = b.totalPlaySec / 60;
  const tempo = playMin > 0 ? (b.totalPoints / playMin).toFixed(1) : '0.0';
  return { ...b, avgPointsPerSet, avgSetDurationSec, tempo };
}

function accumulateGlobalForMatch(bucket, m) {
  bucket.matches++;
  if (m.status === 'active') bucket.activeMatches++;
  if (m.status === 'finished') bucket.finishedMatches++;
  if (m.result === 'draw') bucket.draws++;
  const sets = (m.sets || []).filter(s => s.status !== 'live');
  bucket.totalSets += sets.length;
  sets.forEach(s => {
...
function pickGlobalStatsBucket(g, formatView = 'combined') {
  if (!g) return finalizeGlobalStatsBucket(createGlobalStatsBucket());
  if (formatView === 'singles') return g.singles;
  if (formatView === 'doubles') return g.doubles;
  return g.combined;
}

function computeGlobalStats() {
  const singles = createGlobalStatsBucket();
  const doubles = createGlobalStatsBucket();
  const combined = createGlobalStatsBucket();
  const playerIds = new Set();
  matches.forEach(m => {
    (m.teamA || []).forEach(id => playerIds.add(id));
    (m.teamB || []).forEach(id => playerIds.add(id));
    const isD = isDoublesMatch(m);
    accumulateGlobalForMatch(combined, m);
    accumulateGlobalForMatch(isD ? doubles : singles, m);
  });
  return {
    totalMatches: matches.length,
    playersActive: playerIds.size,
    singles: finalizeGlobalStatsBucket(singles),
    doubles: finalizeGlobalStatsBucket(doubles),
    combined: finalizeGlobalStatsBucket(combined),
  };
}
...
function computeH2HStats(idA, idB) {
  ...
  const raw = {
    matches: [],
    singles: { a: createParticipantStatsShell(), b: createParticipantStatsShell() },
    doubles: { a: createParticipantStatsShell(), b: createParticipantStatsShell() },
    combined: { a: createParticipantStatsShell(), b: createParticipantStatsShell() },
  };
  matches.forEach(m => {
    ...
    const isD = isDoublesMatch(m);
    accumulateMatchForSide(raw.combined.a, m, sideA, { participantId: idA });
    accumulateMatchForSide(raw.combined.b, m, sideB, { participantId: idB });
    const fmt = isD ? raw.doubles : raw.singles;
    accumulateMatchForSide(fmt.a, m, sideA, { participantId: idA });
    accumulateMatchForSide(fmt.b, m, sideB, { participantId: idB });
  });
  ['singles', 'doubles', 'combined'].forEach(key => {
    raw[key].a = finalizeParticipantStats(raw[key].a);
    raw[key].b = finalizeParticipantStats(raw[key].b);
  });
  return raw;
}
...
function filterMatchesByStatsFormat(matchList, formatView = 'combined') {
  if (!matchList?.length || formatView === 'combined') return matchList || [];
  return matchList.filter(m => (formatView === 'doubles' ? isDoublesMatch(m) : !isDoublesMatch(m)));
}
...
function playerHasVisibleStats(stats) {
  if (stats?.combined) {
    return playerHasVisibleStats(stats.combined)
      || playerHasVisibleStats(stats.singles)
      || playerHasVisibleStats(stats.doubles);
  }
    ...
    if (guest?.isGuest) {
      ...
      return {
        ...
        statsLine: playerHasVisibleStats(stats)
          ? `${combined.matchesPlayed} meczów · ${combined.setsWon} wygranych setów w lidze`
          : null,
        bannerTag: 'Gość → pełne konto',
...
function getInterSetRestSec(m) {
  const periods = m.matchTiming?.breakPeriods || [];
  let sec = periods.reduce((s, x) => s + (x || 0), 0);
  if (isMatchLiveActive(m) && !isMatchEditMode(m) && getTimingPhase(m) === 'inter_break') {
    sec += getInterBreakElapsed(m);
  }
  return sec;
}
...
function computeWarmupSec(m) {
  ...
  if (!hasMatchClockStarted(m)) {
    let w = getPreMatchElapsed(m);
    if (isServeDuelActive(m)) w = Math.max(0, w - getServeDuelElapsed(m));
    return w;
  }
  return 0;
}
...
function buildMatchTimelineSegments(m) {
  ...
  if (m.liveSet) {
    const liveSec = getLiveSetElapsed(m);
    if (liveSec > 0) segments.push({ kind: 'play', sec: liveSec, label: `Set ${m.liveSet.n}` });
  }

  if (isMatchLiveActive(m) && !isMatchEditMode(m) && getTimingPhase(m) === 'inter_break') {
    const br = getInterBreakElapsed(m);
    if (br > 0) segments.push({ kind: 'rest', sec: br, label: 'Przerwa' });
  }

  let total = getMatchClockElapsed(m);
  let segSum = segments.reduce((s, x) => s + x.sec, 0);
  if (total < segSum) total = segSum;
  if (total <= 0) total = Math.max(segSum, 1);

  const gap = total - segSum;
  if (gap > 0) {
    if (segments.length && segments[0].kind === 'warmup') segments[0].sec += gap;
    else segments.unshift({ kind: 'warmup', sec: gap, label: 'Rozgrzewka' });
  }

  const playSec = segments.filter(x => x.kind === 'play').reduce((s, x) => s + x.sec, 0);
  const restSec = segments.filter(x => x.kind === 'rest').reduce((s, x) => s + x.sec, 0);
  const playPct = total > 0 ? Math.round((playSec / total) * 100) : 0;
  const restPct = total > 0 ? Math.round((restSec / total) * 100) : 0;
...
function renderStatHelp(helpId, text) {
  return `
    <span class="stat-help">
      <button type="button" class="stat-help__btn" data-action="toggle-stat-help" data-help-id="${helpId}" aria-label="Wyjaśnienie statystyki">?</button>
      <span class="stat-help__bubble" id="stat-help-${helpId}" hidden>${text}</span>
    </span>`;
}
...
const ARCHIVE_ICON = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M4 7h16M6 7V5h12v2M5 7l1 13h12l1-13"/><path d="M9 11v5M15 11v5"/></svg>`;
...
function adjustLiveScore(m, side, delta) {
  ...
  saveState({ immediatePush: true });
  updateSetPlayDOM(m, { scoresOnly: true });
}
...
function softUpdatePlayerDetail(playerId) {
  ...
  const pointsLabel = statsFormatView === 'singles' ? 'Punkty w singlu'
    : statsFormatView === 'doubles' ? 'Punkty w deblu' : 'Punkty łącznie';
  const timeLabel = statsFormatView === 'singles' ? 'Czas w singlu'
    : statsFormatView === 'doubles' ? 'Czas w deblu' : 'Łączny czas gry';
  const statMap = {
    ...
    'Punkty w singlu': stats.totalPoints,
    'Punkty w deblu': stats.totalPoints,
    [pointsLabel]: stats.totalPoints,
    ...
    'Czas w singlu': formatDuration(stats.totalPlaySec),
    'Czas w deblu': formatDuration(stats.totalPlaySec),
...
function isLiveSetPlayOverlayOpen(m) {
  return openMatchId === m?.id && setPlayOpen && !!m?.liveSet
    && !!document.querySelector('.match-page .set-play-glass');
}
...
function ensureLiveSetRowInList(m) {
  if (!m?.liveSet) return;
  if (document.querySelector(`.set-row--live[data-set-n="${m.liveSet.n}"]`)) return;
  updateSetListFromModel(m);
}

function minimalLiveOverlaySoftUpdate(m, remoteHints = {}) {
  if (!isLiveSetPlayOverlayOpen(m) || !isScoreOnlyRemoteSync(remoteHints)) return false;
...
function renderMatchRefereeChip(m) {
  ...
  if (!name) return '';
  return `<p class="match-referee-chip">Sędzia: <strong>${escAttr(name)}</strong></p>`;
}
...
function renderMatchInviteRow(m) {
  ...
  return `
    <div class="match-invite-row${solo ? ' match-invite-row--solo' : ''}">
      ${showWatch ? `
      ...
      ${showRefereeRequest ? `
      <button class="match-invite-row__btn match-invite-row__btn--referee" data-action="request-referee" type="button">
        ${WHISTLE_ICON}
        Sędziuj mecz
      </button>` : ''}
    </div>`;
...
function renderMatchResumeBtn(m) {
  ...
  if (active && referee) {
    return `<button class="btn btn--primary btn--full match-actions__resume match-actions__resume--referee" data-action="resume-set-play" type="button">Wróć do seta (sędzia)</button>`;
  }
  if (active && !editable) {
...
function updateMatchInviteFromModel(m) {
  const aside = document.querySelector('.match-page__aside');
  if (!aside) return;
  const html = renderMatchInviteRow(m);
  const existing = aside.querySelector('.match-invite-row');
  if (!html) {
    existing?.remove();
    return;
  }
...
function updateMatchRefereeChipFromModel(m) {
  const hero = document.querySelector('.match-detail__hero');
  if (!hero) return;
  const html = renderMatchRefereeChip(m);
  const existing = hero.querySelector('.match-referee-chip');
  if (!html) {
    existing?.remove();
    return;
  }
  if (existing) existing.outerHTML = html;
    ...
    const anchor = hero.querySelector('.match-detail__live')
      || hero.querySelector('.match-detail__archive-tag')
      || hero.querySelector('.match-detail__date');
...
function softUpdateMatchDetail(m, remoteHints = {}) {
  ...
  if (minimalLiveOverlaySoftUpdate(m, remoteHints)) return;

  if ((remoteHints.liveSetStarted || remoteHints.serveDuelStarted) && openMatchId === m.id) {
    if (remoteHints.liveSetStarted && isLiveSetPlayOverlayOpen(m)) {
      ensureLiveSetRowInList(m);
      if (m.liveSet?.status === 'running') ensureSetTimerRunning(m);
      updateSetPlayDOM(m);
      updateMatchDetailLiveBadge(m);
      return;
    ...
    if (isMatchRefereeMode(m)) {
      ...
      if (changed && isServeDuelActive(m) && !document.querySelector('.serve-picker-layer') && !shouldAvoidMatchDetailRender()) {
        if (!patchServePickerOverlay(m)) render();
        return;
      }
    }
  }

  if (isServeDuelActive(m) && isMatchRefereeMode(m) && servePickerMatchId === m.id
      && !servePickerPhase && !isLiveSettling() && !document.querySelector('.serve-picker-layer')
      && openMatchId === m.id && !shouldAvoidMatchDetailRender()) {
    if (!patchServePickerOverlay(m)) render();
    return;
  }

  if (m.liveSet && servePickerMatchId === m.id && !servePickerPhase && !isLiveSettling()
      && !document.querySelector('.serve-picker-layer') && !isServeDuelActive(m)) {
    servePickerMatchId = null;
    sessionStorage.removeItem(SERVE_PICKER_KEY);
    releaseWakeLock();
  }
...
function scheduleLeagueStateUiApply() {
  clearTimeout(leagueUiApplyTimer);
  leagueUiApplyTimer = setTimeout(() => {
    leagueUiApplyTimer = null;
    applyLeagueStateUiFromCloud();
  }, 48);
}
...
function updateSetPlayDOM(m, opts = {}) {
  const ls = m.liveSet;
  if (!ls) return;
  updateLiveScoresDOM(m);
  updateSetPlayClock(m);
  ...
  if (clockWrap) {
    ...
    if ((wantLive || wantBreak) && !badge) {
      ...
      const hasBreak = badge.classList.contains('break-badge') || badge.textContent?.includes('Przerwa');
      if (wantBreak !== hasBreak) badge.outerHTML = badgeHtml;
    } else if (badge && !badgeHtml) {
      badge.remove();
    }
    ...
    if (ctlHtml && !ctl) {
      ...
      const wantsPause = ls.status === 'paused';
      const hasResume = !!ctl.querySelector('[data-action="resume-set-timer"]');
      if (wantsPause !== hasResume) ctl.outerHTML = ctlHtml;
    }
  }
  const mainBtn = document.querySelector('[data-action="finish-live-set"]');
  if (mainBtn) mainBtn.hidden = false;
  if (!opts.scoresOnly) scheduleMatchFaceFit();
}
...
function renderServePickerOverlay(m) {
  ...
  const closeDisabled = confirming || expanding || isRefereeUiLocked(m);
  const referee = isMatchRefereeMode(m);
  const previewM = expanding ? buildServeExpandPreview(m) : null;
  const scoreable = canScoreMatch(m);
  const setStage = expanding && previewM
    ? `<div class="serve-expand-stage serve-expand-stage--set">${renderSetPlayOverlayBody(previewM, { readonly: !scoreable, showTitle: true, referee, showRefereeTag: false })}</div>`
    : '';
  return `
    <div class="overlay-layer serve-picker-layer${confirming ? ' serve-picker-layer--confirm' : ''}${expanding ? ' serve-picker-layer--expand' : ''}${referee ? ' serve-picker-layer--referee' : ''}">
      ...
      <div class="overlay-glass overlay-glass--static serve-expand-shell serve-expand-shell--compact${confirming ? ' serve-expand-shell--confirm' : ''}${referee ? ' serve-expand-shell--referee' : ''}">
...
function renderSetPlayOverlayBody(m, { readonly = false, showTitle = true, referee = false, showRefereeTag = true } = {}) {
  ...
  const refereeMode = referee || isMatchRefereeMode(m);
  return `
        ${refereeMode && showRefereeTag ? '<p class="match-referee-chip match-referee-chip--overlay">Tryb sędziego</p>' : ''}
        ...
        ${readonly ? '' : `
        <button class="btn btn--primary btn--full set-play__save${refereeMode ? ' btn--referee' : ''}" data-action="finish-live-set" type="button">Zakończ set</button>
        ${refereeMode ? '' : `<button class="set-play__cancel" data-action="delete-set" data-set-n="${ls.n}" type="button" aria-label="Anuluj set">${CANCEL_SET_ICON} Anuluj set</button>`}`}`;
}
...
function renderSetPlayOverlay(m) {
  ...
  const referee = isMatchRefereeMode(m);
  return `
    <div class="overlay-layer${referee ? ' overlay-layer--referee' : ''}">
      <div class="overlay-glass overlay-glass--static set-play-glass${referee ? ' set-play-glass--referee' : ''}">
        ${referee ? '' : `<button class="match-info-glass__close set-play-glass__back" data-action="close-set-play" type="button" aria-label="Wróć do meczu">${BACK_ICON}</button>`}
        ${renderSetPlayOverlayBody(m, { readonly, showTitle: true, referee })}
      </div>
    </div>`;
}
...
function renderMatchDetailPage(rawM) {
  ...
  const editable = canEditMatch(m);
  const refereeLocked = isRefereeUiLocked(m);
  const refereeMode = isMatchRefereeMode(m);
...
function statsFormatEmptyMessage(formatView) {
  if (formatView === 'singles') return 'Brak meczów w singlu.';
  if (formatView === 'doubles') return 'Brak meczów w deblu.';
  return 'Brak rozegranych meczów.';
}

function statsFormatRankingHint(formatView) {
  if (formatView === 'singles') return 'Ranking singlowy — wygrane mecze, potem wygrane sety.';
  if (formatView === 'doubles') return 'Ranking deblowy — wygrane mecze, potem wygrane sety.';
  return 'Ranking łączny — wygrane mecze, w przypadku remisu wygrane sety.';
}

function renderParticipantStatsRows(stats, { extended = true, formatView = 'combined' } = {}) {
  if (!playerHasVisibleStats(stats)) {
    return `<p class="match-detail__empty">${statsFormatEmptyMessage(formatView)}</p>`;
  }
  const pointsLabel = formatView === 'singles' ? 'Punkty w singlu'
    : formatView === 'doubles' ? 'Punkty w deblu' : 'Punkty łącznie';
  const timeLabel = formatView === 'singles' ? 'Czas w singlu'
    : formatView === 'doubles' ? 'Czas w deblu' : 'Łączny czas gry';
  const sections = [
    {
      title: 'Mecze i sety',
      rows: [
  ...
  if (extended) {
    ...
    sections.push({
      title: 'Czas',
      rows: [[timeLabel, formatDuration(stats.totalPlaySec)]],
    });
  } else {
    sections.push({
      title: 'Czas i tempo',
...
function renderH2HComparison(idA, idB) {
  ...
  const formatMatches = filterMatchesByStatsFormat(h2h.matches, h2hFormatView);
  const bucket = pickH2HStatsBucket(h2h, h2hFormatView);
  const a = bucket.a;
  const b = bucket.b;
  const formatEmpty = formatMatches.length === 0;
  const subWins = side => {
    if (h2hFormatView !== 'combined') return '';
    const sing = h2h.singles[side].matchesWon;
    const doub = h2h.doubles[side].matchesWon;
    if (!sing && !doub) return '';
...
function sortPlayersForRanking(list, formatView = 'combined') {
  return [...list].sort((a, b) => {
    const sa = pickParticipantStatsBucket(a.stats, formatView);
    const sb = pickParticipantStatsBucket(b.stats, formatView);
    if (sb.matchesWon !== sa.matchesWon) return sb.matchesWon - sa.matchesWon;
...
function renderFlowMatchLoading({ referee = false } = {}) {
  ...
  return `
    <div class="matches-page${referee ? ' match-page--referee-locked' : ''}">
      ...
      ${referee ? '<p class="match-referee-chip">Tryb sędziego</p>' : ''}
...
function renderRefereeNamePrompt() {
  if (!refereeNamePromptOpen || userSession.loggedIn) return '';
  const m = matches.find(x => x.id === pendingRefereeMatchId);
  const metaA = m ? getTeamMeta(m, 'A') : null;
  const metaB = m ? getTeamMeta(m, 'B') : null;
  const headline = m ? `${formatTeam(m.teamA, metaA, m)} vs ${formatTeam(m.teamB, metaB, m)}` : 'Ładowanie meczu…';
  return `
...
function mountRefereeNamePrompt() {
  let root = document.getElementById('referee-name-root');
  if (!root) {
    root = document.createElement('div');
    root.id = 'referee-name-root';
    document.getElementById('app')?.appendChild(root);
  }
  root.innerHTML = renderRefereeNamePrompt();
}
...
function handleGlobalModalClick(e) {
  ...
  if (action === 'pick-server') {
    ...
    const m = matches.find(x => x.id === openMatchId);
    if (m && requireLiveMatchControl(m) && isServeDuelStarter(m)) {
      e.preventDefault();
...
document.addEventListener('click', e => {
  ...
  if (e.target.closest('[data-action="confirm-referee-entry"]')) {
    const name = document.getElementById('referee-display-name')?.value || '';
    confirmRefereeEntry(name);
    return;
  }
...
content?.addEventListener('click', async e => {
  ...
  if (e.target.closest('[data-action="share-referee-invite"]')) {
    const m = matches.find(x => x.id === openMatchId);
    if (!m || !canEditMatch(m) || !isMatchActive(m) || isMatchEditMode(m) || hasActiveReferee(m)) return;
    shareRefereeInvite(m);
    return;
  }

  if (e.target.closest('[data-action="request-referee"]')) {
    const m = matches.find(x => x.id === openMatchId);
    if (!m) return;
    requestRefereeRole(m);
    return;
  }

  if (e.target.closest('[data-action="approve-referee"]')) {
    const m = matches.find(x => x.id === openMatchId);
    if (!m) return;
    approveRefereeRequest(m);
    return;
  }

  if (e.target.closest('[data-action="reject-referee"]')) {
    const m = matches.find(x => x.id === openMatchId);
    if (!m) return;
    rejectRefereeRequest(m);
    return;
  }
  ...
  if (e.target.closest('[data-action="plan-open-maps"]')) {
    e.stopPropagation();
    const placeId = e.target.closest('[data-action="plan-open-maps"]').dataset.placeId;
  ...
  if (e.target.closest('[data-action="ctx-plan-edit"]')) {
    const id = parseInt(e.target.closest('[data-action="ctx-plan-edit"]').dataset.sessionId, 10);
    const session = getPlannedSession(id);
    if (!session || !canManagePlannedSession(session)) return;
    ctxTarget = null;
    openPlannedSessionId = id;
    planEditOpen = true;
    planEditDraft = plannedSessionEditDefault(session);
    render();
    return;
  }

  if (e.target.closest('[data-action="ctx-plan-archive"]')) {
    const id = parseInt(e.target.closest('[data-action="ctx-plan-archive"]').dataset.sessionId, 10);
    const session = findPlannedSessionRaw(id);
    if (!session || !canManagePlannedSession(session)) return;
    ctxTarget = null;
    if (archivePlannedSession(session)) {
      showToast('Zarchiwizowano planowanie', 'success');
      render();
    }
    return;
  }

  if (e.target.closest('[data-action="ctx-plan-delete"]')) {
    const id = parseInt(e.target.closest('[data-action="ctx-plan-delete"]').dataset.sessionId, 10);
    const session = findPlannedSessionRaw(id);
    if (!session) return;
    ctxTarget = null;
    promptDeletePlannedSession(session);
    return;
  }

  if (e.target.closest('[data-action="plan-archive-session"]')) {
    const id = parseInt(e.target.closest('[data-action="plan-archive-session"]').dataset.sessionId, 10);
    const session = findPlannedSessionRaw(id);
    if (!session || !canManagePlannedSession(session)) return;
    if (archivePlannedSession(session)) {
      showToast('Zarchiwizowano planowanie', 'success');
  ...
  if (e.target.closest('[data-action="plan-delete-session"]')) {
    const id = parseInt(e.target.closest('[data-action="plan-delete-session"]').dataset.sessionId, 10);
    const session = findPlannedSessionRaw(id);
    if (!session) return;
    promptDeletePlannedSession(session);
    return;
  ...
  if (e.target.closest('[data-action="set-plan-format"]')) {
    ...
    updateNewPlannedFormatDOM();
    return;
  }

  if (e.target.closest('[data-action="set-plan-edit-format"]')) {
    if (!planEditDraft) return;
    syncPlanEditDraftFromDom();
    planEditDraft.format = e.target.closest('[data-action="set-plan-edit-format"]').dataset.format === 'doubles' ? 'doubles' : 'singles';
    updatePlanEditFormatDOM();
    return;
  }

  if (e.target.closest('[data-action="open-plan-edit"]')) {
    const id = parseInt(e.target.closest('[data-action="open-plan-edit"]').dataset.sessionId, 10);
    const session = getPlannedSession(id);
    if (!session || !canManagePlannedSession(session)) return;
    planEditDraft = plannedSessionEditDefault(session);
    planEditOpen = true;
  ...
  if (e.target.closest('[data-action="close-plan-edit"]')) {
    planEditOpen = false;
    planEditDraft = null;
    render();
    return;
  ...
  if (e.target.closest('[data-action="play-set"]')) {
    const m = matches.find(x => x.id === openMatchId);
    if (!m || !requireLiveMatchControl(m)) return;
  ...
  if (e.target.closest('[data-action="score-a-plus"]')) {
    const m = matches.find(x => x.id === openMatchId);
    if (!m || !requireMatchScore(m)) return;
  ...
  if (e.target.closest('[data-action="toggle-stat-help"]')) {
    const btn = e.target.closest('[data-action="toggle-stat-help"]');
    const id = btn.dataset.helpId;
    const bubble = document.getElementById(`stat-help-${id}`);
    if (!bubble) return;
...
content?.addEventListener('change', e => {
  ...
  if (e.target.matches('[data-action="plan-slot-label"]')) {
    const sessionId = parseInt(e.target.dataset.sessionId, 10);
    const slotId = parseInt(e.target.dataset.slotId, 10);
    if (savePlanSlotLabel(sessionId, slotId, e.target.value)) render();
  }
});
...
async function bootstrap() {
  ...
  try {
    if (typeof BadmintonCloud !== 'undefined') {
      const cloudResult = await BadmintonCloud.init({
        ...
        onAuthChange: (user, signedIn) => {
          ...
          if (!signedIn) {
            if (isWatchFlowActive() || isRefereeFlowActive()) return;
    ...
    void (async () => {
      try {
        await Promise.all([ensureWatchLeagueSync(), ensureRefereeLeagueSync()]);
        handleWatchLeagueSync();
        handleRefereeLeagueSync();
